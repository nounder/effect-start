...
import * as RouteHttp from "../../src/RouteHttp.ts"
import * as BunSql from "../../src/sql/bun/index.ts"
import type * as SqlClient from "../../src/sql/SqlClient.ts"
import * as OpenTelemetry from "../../src/studio/internal/OpenTelemetry.ts"
...
const captures: ReadonlyArray<Capture> = NFs
  .readFileSync(NPath.join(import.meta.dir, "fixtures", "otlp-traffic.ndjson"), "utf8")
  .split("\n")
...
const sqlLayer = BunSql.layer({
  adapter: "sqlite",
  filename: ":memory:",
  safeIntegers: true,
})

function run<A>(effect: Effect.Effect<A, unknown, Studio.Studio | SqlClient.SqlClient>) {
  return effect.pipe(
...
function replay(capture: Capture) {
  return Effect.gen(function*() {
    ...
    const runtime = yield* Effect.runtime<Studio.Studio | SqlClient.SqlClient>()
