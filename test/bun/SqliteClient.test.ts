...
test.describe("SqliteClient", () => {
        ...
        Effect.gen(function*() {
          ...
          yield* sql.withTransaction(
            Effect.gen(function*() {
              ...
              test
                .expect(Result.isFailure(spResult))
                .toBe(true)
            }),
          )
            ...
            Effect.gen(function*() {
              ...
              yield* sql.withTransaction(
                Effect.gen(function*() {
                  ...
                  test
                    .expect(Result.isFailure(innerResult))
                    .toBe(true)
                }),
              )
            }),
  ...
  test.describe("error handling", () => {
        ...
        Effect.gen(function*() {
          ...
          if (Result.isFailure(result)) {
            test
              ...
              .toBe("SqlError")
            test
              .expect(["UniqueViolation", "ConstraintError"])
              .toContain(result.failure.reason._tag)
          }
        }),
      ))
    ...
    test.it("should include original error as cause", () =>
      runSql(
        Effect.gen(function*() {
          const sql = yield* Sql.SqlClient
          const result = yield* sql`INVALID SQL SYNTAX`.pipe(Effect.result)
        ...
        Effect.gen(function*() {
          ...
          yield* sql.withTransaction(
            Effect.gen(function*() {
              ...
              test
                .expect(Result.isFailure(inner))
                .toBe(true)

              if (Result.isFailure(inner)) {
                test
                  .expect(inner.failure._tag)
                  .toBe("SqlError")
              }
            }),
          )
    ...
    test.it("should interpolate identifier, list, and values fragments", () =>
      runSql(
        Effect.gen(function*() {
          ...
          const rows = yield* sql<{
            name: string
          }>`SELECT name FROM ${table} WHERE name IN ${sql.in(["Alice"])} ORDER BY name`
  ...
  test.describe("statements", () => {
    test.it("tagged templates compile SQL and parameters", () =>
      runSql(
        Effect.gen(function*() {
          const sql = yield* Sql.SqlClient
          const statement = sql`SELECT * FROM users WHERE name = ${"Alice"}`

          test
            .expect(statement.compile())
            .toEqual(["SELECT * FROM users WHERE name = ?", ["Alice"]])
        }),
      ))

    test.it("unsafe statements compile SQL and parameters", () =>
      runSql(
        Effect.gen(function*() {
          const sql = yield* Sql.SqlClient
          const statement = sql.unsafe("SELECT * FROM users WHERE id = ?", [42])

          test
            .expect(statement.compile())
            .toEqual(["SELECT * FROM users WHERE id = ?", [42]])
        }),
      ))

    test.it("unsafe statements default to empty parameters", () =>
      runSql(
        Effect.gen(function*() {
          const sql = yield* Sql.SqlClient

          test
            .expect(sql.unsafe("SELECT 1").compile())
            .toEqual(["SELECT 1", []])
        }),
      ))
  })
