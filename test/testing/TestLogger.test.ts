...
test.it("TestLogger captures log messages", () =>
  Effect.gen(function* () {
    ...
    test.expect(messages).toHaveLength(3)
    test.expect(messages[0]).toContain("[Error]")
    test.expect(messages[0]).toContain("This is an error")
    test.expect(messages[1]).toContain("[Warning]")
    test.expect(messages[1]).toContain("This is a warning")
    test.expect(messages[2]).toContain("[Info]")
    test.expect(messages[2]).toContain("This is info")
...
test.it("TestLogger starts with empty messages", () =>
  Effect.gen(function* () {
    ...
    test.expect(messages).toHaveLength(0)
  }).pipe(Effect.provide(TestLogger.layer()), Effect.runPromise),
)
