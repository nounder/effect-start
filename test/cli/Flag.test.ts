  ...
  test.it("string", () =>
    Effect.gen(function*() {
      ...
      const cmd = Command.make("test", {
        ...
        handler: (cfg) => Effect.sync(() => { result = cfg.output }),
      })
  ...
  test.it("boolean defaults to false when absent", () =>
    Effect.gen(function*() {
      ...
      const cmd = Command.make("test", {
        ...
        handler: (cfg) => Effect.sync(() => { result = cfg.verbose }),
      })
      yield* parse(cmd, [])
  ...
  test.it("choice", () =>
    Effect.gen(function*() {
      ...
      const cmd = Command.make("test", {
        ...
        handler: (cfg) => Effect.sync(() => { result = cfg.level }),
      })
  ...
  test.it("variadic flag", () =>
    Effect.gen(function*() {
      ...
      const cmd = Command.make("test", {
        ...
        handler: (cfg) => Effect.sync(() => { result = cfg.tags }),
      })
