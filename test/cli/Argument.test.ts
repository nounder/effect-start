...
const parse = (
  cmd: Command.Command<any, any, any, any>,
  ...
  test.it("optional returns None when missing", () =>
    Effect
      .gen(function*() {
        ...
        test
          .expect(Option.isNone(result as Option.Option<string>))
          .toBe(true)
      })
      .pipe(Effect.runPromise))
  ...
  test.it("optional returns Some when present", () =>
    Effect
      .gen(function*() {
        ...
        test
          .expect(Option.getOrNull(result as Option.Option<string>))
          .toBe(
            "hello",
          )
      })
  ...
  test.it("withDefault", () =>
    Effect
      .gen(function*() {
        ...
        const cmd = Command.make("test", {
          config: {
            name: Argument.string("name").pipe(Argument.withDefault("world")),
          },
          handler: (cfg) =>
            Effect.sync(() => {
              result = cfg.name
...
test.describe("Argument types", () => {
  test.it("infers optional types", () => {
    Command.make("test", {
      ...
      handler: (cfg) => {
        test
          .expectTypeOf(cfg.name)
          .toEqualTypeOf<Option.Option<string>>()
