...
test.describe("generateCode", () => {
  ...
  test.it("generates valid TypeScript with satisfies", () => {
    ...
    test
      .expect(code)
      .toContain(`} satisfies FileRouter.FileRoutes`)
  })

  test.it("generates declare module for registered routes", () => {
    const routes = getRoutes(["route.tsx", "users/[userId]/route.tsx"])

    const code = FileRouterCodegen.generateCode(routes)

    test
      .expect(code)
      .toContain(`declare module "effect-start/dev"`)
    test
      .expect(code)
      .toContain(
