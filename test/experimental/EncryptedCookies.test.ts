  ...
  test.test("return encrypted string in correct format", () =>
    Effect
      .gen(function*() {
        ...
        const result = yield* EncryptedCookies.encrypt(value, {
          secret: "test-secret",
        })
  ...
  test.test("produce different results for same input due to random IV", () =>
    Effect
      .gen(function*() {
        ...
        const result1 = yield* EncryptedCookies.encrypt(value, {
          secret: "test-secret",
        })
        const result2 = yield* EncryptedCookies.encrypt(value, {
          secret: "test-secret",
        })
  ...
  test.test("handle object with undefined properties", () =>
    Effect
      .gen(function*() {
        ...
        const encrypted = yield* EncryptedCookies.encrypt(value, {
          secret: "test-secret",
        })
...
test.describe(`${EncryptedCookies.decrypt.name}`, () => {
  test.test("decrypt encrypted string successfully", () =>
    Effect
      .gen(function*() {
        ...
        const encrypted = yield* EncryptedCookies.encrypt(originalValue, {
          secret: "test-secret",
        })
        const decrypted = yield* EncryptedCookies.decrypt(encrypted, {
          secret: "test-secret",
  ...
  test.test("fail with invalid format", () => {
    ...
    test
      .expect(
        Effect.runPromise(EncryptedCookies
          .decrypt(invalidValue, { secret: "test-secret" })),
      )
      .rejects
      .toThrow()
  })
  ...
  test.test("fail with null value", () => {
    test
      .expect(
        Effect.runPromise(EncryptedCookies
          .encrypt(null, { secret: "test-secret" })),
      )
      .rejects
      .toThrow()
  })
  ...
  test.test("fail with undefined value", () => {
    test
      .expect(
        Effect.runPromise(EncryptedCookies
          .encrypt(undefined, { secret: "test-secret" })),
      )
      .rejects
      .toThrow()
  })
  ...
  test.test("preserve cookie properties and decrypt value", () =>
    Effect
      .gen(function*() {
        ...
        const decrypted = yield* EncryptedCookies.decryptCookie(encrypted, {
          secret: "test-secret",
        })
