      ...
      runSql(
        Effect.gen(function*() {
          ...
          test
            ...
            .toHaveLength(5)
          test
            .expect(users.columns[0].columnName)
            .toBe("id")
          test
            .expect(users.columns[0].isPrimaryKey)
            .toBe(true)
          test
            .expect(users.columns[0].isAutoIncrement)
            .toBe(true)
          test
            .expect(users.columns[1].columnName)
            .toBe("name")
          test
            .expect(users.columns[1].isNullable)
            .toBe(false)
          test
            .expect(users.columns[2].columnName)
            .toBe("email")
          test
            .expect(users.columns[2].isNullable)
            .toBe(true)
          test
            .expect(users.columns[3].columnDefault)
            .toBe("0")
          ...
          test
            ...
            .toHaveLength(5)
          test
            .expect(posts.foreignKeys)
            .toHaveLength(1)
          test
            .expect(posts.foreignKeys[0].referencedTable)
            .toBe("users")
          test
            .expect(posts.foreignKeys[0].referencedColumn)
            .toBe("id")
          test
            .expect(posts.foreignKeys[0].columnName)
            .toBe("user_id")
          test
            .expect(posts.foreignKeys[0].deleteRule)
            .toBe("CASCADE")

          test
            .expect(posts.indexes)
            .toHaveLength(1)
          test
            .expect(posts.indexes[0].indexName)
            .toBe("idx_posts_user_id")
          test
            .expect(posts.indexes[0].isUnique)
            .toBe(false)

          test
            .expect(users.indexes)
            .toHaveLength(1)
          test
            .expect(users.indexes[0].indexName)
            .toBe("idx_users_email")
          test
            .expect(users.indexes[0].isUnique)
            .toBe(true)
        }),
      ))
      ...
      runSql(
        Effect.gen(function*() {
          ...
          test
            ...
            .toHaveLength(2)
          test
            .expect(ts.columns.map((c) => c.columnName))
            .toEqual([
              "id",
              "name",
            ])
    ...
    test.it("sortableColumns should list sortable column names", () =>
      runSql(
        Effect.gen(function*() {
          ...
          test
            .expect([...users.sortableColumns].sort())
            .toEqual([
              "email",
              "id",
            ])
          ...
          test
            .expect([...posts.sortableColumns].sort())
            .toEqual([
              "id",
              "user_id",
            ])
    ...
    test.it("findAll should sort ascending by default", () =>
      runSql(
        Effect.gen(function*() {
          ...
          test
            .expect((rows[0] as any).id)
            .toBe(1)
          test
            .expect((rows[2] as any).id)
            .toBe(3)
        }),
      ))
    ...
    test.it("findAll should support multiple sort columns", () =>
      runSql(
        Effect.gen(function*() {
          ...
          test
            .expect((rows[0] as any).category)
            .toBe(1)
          test
            .expect((rows[0] as any).name)
            .toBe("D")
          test
            .expect((rows[1] as any).category)
            .toBe(1)
          test
            .expect((rows[1] as any).name)
            .toBe("A")
          test
            .expect((rows[2] as any).category)
            .toBe(2)
        }),
      ))
        ...
        runSql(
          Effect.gen(function*() {
            ...
            test
              ...
              .toHaveLength(2)
            test
              .expect((rows as Array<any>).every((r) => r.name !== "Alice"))
              .toBe(true)
          }),
        ))
    ...
    test.it("table should return undefined for unknown table", () =>
      runSql(
        Effect.gen(function*() {
          ...
          test
            .expect(reader.table("nonexistent"))
            .toBeUndefined()
        }),
      ))
    ...
    test.it("should list all table readers", () =>
      runSql(
        Effect.gen(function*() {
          ...
          test
            .expect(reader.tables)
            .toHaveLength(2)
          test
            .expect(reader.tables.map((t) => t.tableName).sort())
            .toEqual([
              "posts",
              "users",
            ])
