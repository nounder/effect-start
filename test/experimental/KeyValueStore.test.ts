  ...
  test.it("set", () =>
    Effect
      .gen(function*() {
        ...
        test
          .expect(yield* kv.get("/foo/bar"))
          .toBe("bar")
        test
          .expect(yield* kv.size)
          .toBe(1)
      })
      .pipe(
        Effect.provide(KeyValueStore.layerMemory),
  ...
  test.it("modify", () =>
    Effect
      .gen(function*() {
        ...
        test
          .expect(yield* kv.modify("foo", (v) => v + "bar"))
          .toBe("barbar")
        test
          .expect(yield* kv.size)
          .toBe(1)
  ...
  test.it("prefixes the keys", () =>
    Effect
      .gen(function*() {
        ...
        test
          .expect(yield* prefixed.get("foo"))
          .toBe("barbar")
        test
          .expect(yield* prefixed.has("foo"))
          .toBe(true)

        test
          .expect(yield* store.get("prefix/foo"))
          .toBe("barbar")
        test
          .expect(yield* store.has("prefix/foo"))
          .toBe(true)
      })
      .pipe(
        Effect.provide(KeyValueStore.layerMemory),
