...
import * as Unique from "effect-start/Unique"
  ...
  test.it("returns a canonical UUIDv7 string", () => {
    test
      .expect(
        withNow(0, () =>
          withRandomValues(
            new Uint8Array([0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x11, 0x22]),
            () => Unique.uuid7(),
          ),
        ),
      )
...
test.describe("Unique.ulid", () => {
  test.it("returns a base32 ULID with a timestamp prefix", () => {
    test
      .expect(
        withRandomValues(
          new Uint8Array([0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f]),
  ...
  test.it("does not mix random bits into the timestamp prefix", () => {
    ...
    const ulidB = withRandomValues(
      new Uint8Array([0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
      () => Unique.ulid(time),
    )
...
test.describe("Unique.token", () => {
  test.it("returns a base32 token with the requested length", () => {
    test
      .expect(withRandomValues(new Uint8Array([0x00, 0x01, 0x02, 0x03]), () => Unique.token(4)))
      .toBe("0123")

    test.expect(withRandomValues(new Uint8Array(128), () => Unique.token())).toBe("0".repeat(32))
  })
})
