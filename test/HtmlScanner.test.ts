...
test.describe("parse", () => {
  test.it("finds script tags with attributes", () => {
    const els = [
      ...HtmlScanner.parse(
        `<html><head><script src="/app.js" type="module"></script></head></html>`,
      ),
    ]
    const script = els.find((e) => e.tagName === "script")!

    test
      .expect(script.attributes.get("src"))
      .toBe("/app.js")
    test
      .expect(script.attributes.get("type"))
      .toBe("module")
  })

  test.it("finds link tags", () => {
    const els = [
      ...HtmlScanner.parse(`<link rel="stylesheet" href="/style.css">`),
    ]

    test
      .expect(els[0].tagName)
      .toBe("link")
    test
      .expect(els[0].attributes.get("rel"))
      .toBe("stylesheet")
    test
      .expect(els[0].attributes.get("href"))
      .toBe("/style.css")
  })
  ...
  test.it("yields all tags", () => {
    const els = [
      ...HtmlScanner.parse(
        `<div class="foo"><p>hello</p><script src="x"></script></div>`,
      ),
    ]
    ...
    test
      .expect(tags)
      .toEqual(["div", "p", "script"])
  })

  test.it("handles self-closing tags", () => {
    const els = [
      ...HtmlScanner.parse(`<link rel="stylesheet" href="/a.css" />`),
    ]
  ...
  test.it("handles single-quoted attributes", () => {
    ...
    test
      .expect(els[0].attributes.get("src"))
      .toBe("/app.js")
  })
  ...
  test.it("handles boolean attributes", () => {
    const els = [
      ...HtmlScanner.parse(`<script defer async src="/app.js"></script>`),
    ]

    test
      .expect([...els[0].attributes])
      .toEqual([
        ["defer", ""],
        ["async", ""],
        ["src", "/app.js"],
  ...
  test.it("normalizes attribute names to lowercase", () => {
    const els = [
      ...HtmlScanner.parse(`<script SRC="/app.js" TYPE="module"></script>`),
    ]
    ...
    test
      ...
      .toBe("/app.js")
    test
      .expect(els[0].attributes.get("type"))
      .toBe("module")
  })
  ...
  test.it("skips doctype", () => {
    const els = [
      ...HtmlScanner.parse(`<!DOCTYPE html><script src="/app.js"></script>`),
    ]
  ...
  test.it("does not parse tags inside script body", () => {
    const els = [
      ...HtmlScanner.parse(
        `<script src="/app.js">var x = "<link href=trap>";</script><link rel="stylesheet" href="/style.css">`,
      ),
    ]
  ...
  test.it("ignores closing-tag lookalikes inside raw text", () => {
    const els = [
      ...HtmlScanner.parse(
        `<script src="/app.js">const x = "</scriptx>";</script><link rel="stylesheet" href="/style.css">`,
      ),
    ]
  ...
  test.it("skips content inside style tags", () => {
    const els = [
      ...HtmlScanner.parse(
        `<link rel="stylesheet" href="/a.css"><style>body { background: url("<link href=trap>") }</style><script src="/app.js"></script>`,
      ),
    ]
    const tags = els.map((e) => e.tagName)
    ...
    test
      .expect(tags)
      .toEqual(["link", "style", "script"])
  })

  test.it("handles attributes with equals in value", () => {
    const els = [
      ...HtmlScanner.parse(`<script src="/app.js?v=123&t=abc"></script>`),
    ]

    test
      .expect(els[0].attributes.get("src"))
      .toBe("/app.js?v=123&t=abc")
  })
  ...
  test.it("handles mixed quote styles and boolean attrs in one tag", () => {
    const els = [
      ...HtmlScanner.parse(`<script src="a.js" data-x='b' async></script>`),
    ]
  ...
  test.it("works as lazy generator", () => {
    ...
    for (
      const el of HtmlScanner.parse(
        `<div><script src="a"></script><link href="b"><p></p>`,
      )
    ) {
...
test.describe("rewrite", () => {
  ...
  test.it("multiple mutations on different elements", () => {
    ...
    test
      .expect(doc.toString())
      .toBe(
        `<link rel="stylesheet" href="/bundled.css"><script src="/bundled.js"></script>`,
      )
  })

  test.it("multiple setAttribute on same element", () => {
    const doc = HtmlScanner.rewrite(
      `<script src="/old.js" type="module"></script>`,
    )
    for (const el of doc) {
      el.setAttribute("src", "/new.js")
