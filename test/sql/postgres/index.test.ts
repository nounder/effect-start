...
import * as Docker from "../../../src/internal/Docker.ts"
...
const SqlLayer = (() => {
  const container = Effect
    ...
    .pipe(
      Layer.unwrapEffect,
      Layer.provideMerge(Docker.layer),
    )
  ...
  const ready = Layer
    .effectDiscard(
      Effect.flatMap(Docker.DockerContainer, (c) =>
        c
          .exec(["psql", "-U", "postgres", "-d", "test", "-c", "SELECT 1"])
          .pipe(
            ...
            Effect.timeoutFail({
              duration: "30 seconds",
              onTimeout: () =>
                new Docker.DockerError({
                  message: "Timed out waiting for PostgreSQL",
                }),
            }),
            Effect.asVoid,
          )),
  ...
  return Effect
    ...
    .pipe(
      Layer.unwrapEffect,
      Layer.provide(Layer.merge(container, ready)),
    )
})()

const runtime = ManagedRuntime.make(
  SqlLayer.pipe(Layer.provide(BunChildProcessSpawner.layer)),
)
    ...
    test.it("should execute raw SQL strings", () =>
      runSql(
        Effect.gen(function*() {
          ...
          yield* sql.unsafe(
            "CREATE TABLE raw_test (id SERIAL PRIMARY KEY, name TEXT)",
          )
    ...
    test.it("should execute unsafe with parameter values", () =>
      runSql(
        Effect.gen(function*() {
          ...
          yield* sql.unsafe(
            "INSERT INTO unsafe_params (name, age) VALUES ($1, $2)",
            ["Alice", 30],
          )
    ...
    test.it("should execute unsafe inside a transaction", () =>
      runSql(
        Effect.gen(function*() {
          ...
          yield* sql.unsafe(
            "CREATE TABLE unsafe_tx (id SERIAL PRIMARY KEY, name TEXT)",
          )
  ...
  test.describe("error handling", () => {
    test.it("should produce SqlError for invalid queries", () =>
      runSql(
        Effect.gen(function*() {
          const sql = yield* SqlClient.SqlClient
          const result = yield* sql`SELECT * FROM nonexistent_table_xyz`.pipe(
            Effect.either,
          )
    ...
    test.it("should pick specific columns", () =>
      runSql(
        Effect.gen(function*() {
          ...
          const helper = sql({ name: "Charlie", age: 30, email: "c@d.com" }, [
            "name",
            "age",
          ])
          ...
          test
            .expect(rows)
            .toEqual([{ name: "Charlie", age: 30, email: null }])
      ...
      runSql(
        Effect
          .gen(function*() {
            ...
            yield* reserved.unsafe(
              "CREATE TABLE reserve_test (id SERIAL PRIMARY KEY, name TEXT)",
            )
            yield* reserved.unsafe(
              "INSERT INTO reserve_test (name) VALUES ('reserved')",
            )
            const rows = yield* reserved<
              { name: string }
            >`SELECT name FROM reserve_test`
      ...
      runSql(
        Effect
          .gen(function*() {
            ...
            yield* reserved.unsafe(
              "CREATE TABLE reserve_iso (id SERIAL PRIMARY KEY, val INT)",
            )
    ...
    test.it("should stream notifications via PubSub", () =>
      runSql(
        Effect.gen(function*() {
          ...
          test
            .expect(Chunk.toReadonlyArray(result))
            .toEqual(["a", "b", "c"])
