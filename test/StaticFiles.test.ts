  ...
  Effect
    .gen(function*() {
      ...
      yield* fs.writeFileString(
        NPath.join(directory, "nested", "hello.txt"),
        "hello from assets",
      )
      ...
      const entity = yield* client.get(
        "http://localhost/assets/nested/hello.txt",
      )
      ...
      test
        .expect(entity.headers["content-length"])
        .toBe(
          String("hello from assets".length),
        )
      test
        .expect(yield* entity.text)
        .toBe("hello from assets")
    })
    .pipe(
      Effect.scoped,
...
test.it("blocks directory traversal outside the configured directory", () =>
  Effect
    .gen(function*() {
      ...
      yield* fs.writeFileString(
        NPath.join(directory, "secret.txt"),
        "top secret",
      )
      ...
      const client = Fetch.fromHandler(handler)
      const entity = yield* client.get(
        "http://localhost/assets/%2e%2e/secret.txt",
      )
      ...
      yield* Effect
        .gen(function*() {
          const runtime = yield* Effect.runtime<
            Route.Routes | FileSystem.FileSystem
          >()
          const routes = yield* Route.Routes
          const handles = Object.fromEntries(
            RouteHttp.walkHandles(routes, runtime),
          )
...
test.it("decodes URL-encoded characters in path params", () =>
  Effect
    .gen(function*() {
      ...
      yield* fs.writeFileString(
        NPath.join(directory, "hello world.txt"),
        "spaced",
      )
      ...
      const spaced = yield* client.get(
        "http://localhost/assets/hello%20world.txt",
      )
