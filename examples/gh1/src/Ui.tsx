...
export function ErrorPage(props: { title: string; message?: string; backHref?: string; backLabel?: string }) {
  return (
    <Layout>
      <div class="pt-16 text-center">
        ...
        {props.backHref && (
          <a href={props.backHref} class="text-[#58a6ff] hover:underline">{props.backLabel ?? "Go back"}</a>
        )}
      </div>
    </Layout>
  )
    ...
    <div class="min-h-screen bg-[#0d1117] text-[#e6edf3]">
      ...
      <footer class="border-t border-[#21262d] mt-12 py-8 text-center text-xs text-[#8b949e]">
        <div class="max-w-[1280px] mx-auto px-4">
          Built with <a href="https://github.com/effect-ts/effect" class="text-[#58a6ff] hover:underline">Effect</a> + <a href="https://github.com/nikitavoloboev/effect-start" class="text-[#58a6ff] hover:underline">effect-start</a>
        </div>
      </footer>
    </div>
  )
    ...
    <nav class="bg-[#161b22] border-b border-[#21262d] px-4 sm:px-6 lg:px-8 py-3">
      <div class="max-w-[1280px] mx-auto flex items-center gap-4">
        ...
        <div class="flex items-center gap-4 text-sm">
          <a href="/" class="text-[#e6edf3] hover:text-white">Explore</a>
        </div>
      </div>
    </nav>
  )
    ...
    <div class="border border-[#21262d] rounded-md p-4 hover:border-[#30363d] transition-colors">
      <div class="flex items-start justify-between gap-2 mb-1">
        <h3 class="text-[#58a6ff] font-semibold text-sm truncate">
          <a href={`/${props.fullName}`} class="hover:underline">{props.fullName}</a>
        </h3>
        <span class="text-[#8b949e] border border-[#30363d] rounded-full text-xs px-2 py-0.5 shrink-0">Public</span>
      </div>
      {props.description && (
      ...
      <div class="flex items-center gap-4 text-xs text-[#8b949e]">
        {props.language && (
          <span class="flex items-center gap-1">
            <span class="w-3 h-3 rounded-full inline-block" style={`background-color: ${Github.langColor(props.language)}`} />
            {props.language}
          </span>
        )}
        {props.stars > 0 && (
        ...
        {props.updated && (
          <span>Updated {Github.timeAgo(props.updated)}</span>
        )}
      </div>
    </div>
  )
      ...
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1 min-w-0">
          <h3 class="text-xl mb-1">
            <a href={`/${props.fullName}`} class="text-[#58a6ff] font-semibold hover:underline">{props.fullName}</a>
          </h3>
          {props.description && (
...
export function UserCard(props: {
  ...
  return (
    <a href={href} class="flex items-center gap-3 p-3 rounded-md hover:bg-[#161b22] transition-colors">
      <img src={props.avatar} alt={props.login} class={`w-10 h-10 ${props.type === "Organization" ? "rounded-md" : "rounded-full"} bg-[#21262d]`} />
      <div class="min-w-0">
    ...
    <div class="flex items-start gap-2 py-2 px-3 hover:bg-[#161b22] rounded">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <a href={`/${props.repoPath}/issues/${props.number}`} class="text-sm font-semibold text-[#e6edf3] hover:text-[#58a6ff]">
            {props.title}
          </a>
...
export function CommitRow(props: {
  ...
  return (
    <div class="flex items-center gap-3 py-2 px-3 hover:bg-[#161b22] rounded">
      ...
      <div class="flex-1 min-w-0">
        <a href={`/${props.repoPath}/commit/${props.sha}`} class="text-sm text-[#e6edf3] font-medium hover:text-[#58a6ff] truncate block">{firstLine}</a>
        <div class="text-xs text-[#8b949e]">
          <span class="font-medium text-[#e6edf3]">{props.author}</span> committed {Github.timeAgo(props.date)}
        </div>
      </div>
...
export function ContributorCard(props: {
  ...
  return (
    <a href={`/${props.login}`} class="flex items-center gap-2 p-2 rounded hover:bg-[#161b22] transition-colors">
...
export function Tabs(props: { items: Array<{ label: string; href: string; count?: number; active?: boolean }> }) {
  return (
      ...
      {props.items.map((item) => (
        ...
        >
          ...
          {item.count !== undefined && (
            <span class="bg-[#21262d] text-[#8b949e] rounded-full text-xs px-2 py-0.5">{Github.num(item.count)}</span>
          )}
        </a>
      ))}
    </div>
    ...
    <div>
      ...
      <div class="flex flex-wrap gap-3 text-xs">
        {sorted.map(([lang, bytes]) => {
          ...
          return (
            <span class="flex items-center gap-1">
              <span class="w-2 h-2 rounded-full inline-block" style={`background-color: ${Github.langColor(lang)}`} />
...
export function StateFilter(props: { current: string; base: string; counts?: { open?: number; closed?: number } }) {
  return (
    <div class="flex gap-2 mb-4 text-sm">
      ...
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="#a371f7"><path d="M11.28 6.78a.75.75 0 0 0-1.06-1.06L8 7.94 5.78 5.72a.75.75 0 0 0-1.06 1.06L6.94 9l-2.22 2.22a.75.75 0 1 0 1.06 1.06L8 10.06l2.22 2.22a.75.75 0 1 0 1.06-1.06L9.06 9l2.22-2.22Z" /><path d="M8 0a8 8 0 1 1 0 16A8 8 0 0 1 8 0ZM1.5 8a6.5 6.5 0 1 0 13 0 6.5 6.5 0 0 0-13 0Z" /></svg>
        {props.counts?.closed !== undefined ? `${Github.num(props.counts.closed)} Closed` : "Closed"}
      </a>
    </div>
  )
}
