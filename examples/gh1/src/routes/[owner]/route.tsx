      ...
      <Layout>
        <div class="flex flex-col lg:flex-row gap-8 pt-4">
          <aside class="lg:w-[296px] shrink-0">
            <img src={user.avatar_url} class="w-[296px] h-[296px] rounded-full border-2 border-[#21262d] mb-4" />
            ...
            <div class="flex items-center gap-2 text-sm text-[#8b949e] mb-4">
              ...
              <a href={`/${owner}?tab=followers`} class="hover:text-[#58a6ff]">
                <span class="font-semibold text-[#e6edf3]">{Github.num(user.followers)}</span> followers
              </a>
              <span>·</span>
              <a href={`/${owner}?tab=following`} class="hover:text-[#58a6ff]">
                <span class="font-semibold text-[#e6edf3]">{Github.num(user.following)}</span> following
              </a>
            </div>
            ...
            <div class="flex flex-col gap-1 text-sm text-[#8b949e]">
              ...
              {user.blog && (
                <div class="flex items-center gap-2">
                  ...
                  <a href={user.blog.startsWith("http") ? user.blog : `https://${user.blog}`} class="text-[#58a6ff] hover:underline truncate">{user.blog}</a>
                </div>
              )}
              {user.twitter_username && (
                <div class="flex items-center gap-2">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M9.294 6.928 14.357 1h-1.2L8.762 6.147 5.25 1H1.2l5.31 7.784L1.2 15h1.2l4.642-5.436L10.751 15h4.05L9.294 6.928ZM7.651 8.852l-.538-.775L2.832 1.91h1.843l3.454 4.977.538.775 4.491 6.47h-1.843L7.651 8.852Z" /></svg>
                  <a href={`https://x.com/${user.twitter_username}`} class="text-[#58a6ff] hover:underline">@{user.twitter_username}</a>
                </div>
              )}
            </div>
          </aside>
          ...
          <div class="flex-1 min-w-0">
            <Tabs items={[
              { label: "Repositories", href: `/${owner}`, count: user.public_repos, active: tab === "repositories" },
  ...
  return (
    <Layout>
      <div class="pt-6">
        <div class="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <div class="flex items-center gap-4 text-sm text-[#8b949e] mt-2">
              ...
              {org.blog && (
                <a href={org.blog.startsWith("http") ? org.blog : `https://${org.blog}`} class="text-[#58a6ff] hover:underline">{org.blog}</a>
              )}
        ...
        <Tabs items={[
          { label: "Repositories", href: `/orgs/${org.login}`, count: org.public_repos, active: true },
        ]} />
