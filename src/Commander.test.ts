...
test.describe("boolean options", () => {
  test.it(
    "should return false for boolean flag when not specified",
    async () => {
      const cmd = Commander.make({ name: "app" }).option(
        Commander.option("--verbose", "-v")
          .default(false)
...
test.describe("options with defaults", () => {
  ...
  test.it("should handle multiple defaults", async () => {
    const cmd = Commander.make({ name: "app" })
      .option(
        Commander.option("--host").default("localhost").schema(Schema.String),
      )
      .option(
...
test.describe("complex scenarios", () => {
  ...
  test.it("should handle repeatable options", async () => {
    const cmd = Commander.make({ name: "app" }).option(
      Commander.option("--tags", "-t").schema(
        Commander.repeatable(Schema.String),
      ),
    )
