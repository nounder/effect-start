...
test.describe(RouteMatch.match, () => {
  test.it("matches exact static path", () => {
    ...
    const result = RouteMatch.match(Route.items(routes), { path: "/about" })
  ...
  test.it("returns null for unmatched path", () => {
    ...
    const result = RouteMatch.match(Route.items(routes), { path: "/contact" })
  ...
  test.it("matches root path", () => {
    ...
    const result = RouteMatch.match(Route.items(routes), { path: "/" })
  ...
  test.it("omits optional param when absent", () => {
    ...
    const result = RouteMatch.match(Route.items(routes), { path: "/files" })
  ...
  test.it("static > dynamic > required wildcard > optional wildcard priority", () => {
    ...
    const staticMatch = RouteMatch.match(Route.items(routes), {
      path: "/items/special",
    })
  ...
  test.it("matches nested mounted paths", () => {
    ...
    const usersMatch = RouteMatch.match(Route.items(routes), {
      path: "/admin/users",
    })
    const settingsMatch = RouteMatch.match(Route.items(routes), {
      path: "/admin/settings",
    })
...
test.describe(RouteMatch.matchRequest, () => {
  test.it("extracts path from URL", () => {
    ...
    const result = RouteMatch.matchRequest(Route.items(routes), request)
...
test.describe("content negotiation", () => {
  ...
  test.it("selects format by Accept header", () => {
    const textResult = RouteMatch.match(Route.items(multiFormatRoutes), {
      path: "/page",
      headers: { accept: "text/plain" },
    })
    const htmlResult = RouteMatch.match(Route.items(multiFormatRoutes), {
      path: "/page",
      headers: { accept: "text/html" },
    })
    const jsonResult = RouteMatch.match(Route.items(multiFormatRoutes), {
      path: "/page",
  ...
  test.it("respects quality values", () => {
    const result = RouteMatch.match(Route.items(multiFormatRoutes), {
      path: "/page",
  ...
  test.it("returns first format when Accept is missing or */*", () => {
    const noAccept = RouteMatch.match(Route.items(multiFormatRoutes), {
      path: "/page",
    })
    const wildcard = RouteMatch.match(Route.items(multiFormatRoutes), {
      path: "/page",
      headers: { accept: "*/*" },
    })
