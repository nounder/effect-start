...
import * as NodeSink from "./internal/NodeSink.ts"
import * as NodeStream from "./internal/NodeStream.ts"
...
const toPlatformError = (
  ...
  return NodeFileSystem.handleErrnoException("ChildProcess", method)(error, [commandStr])
}
...
const taskkill = (
  ...
  onExit: (error: NodeChildProcess.ExecFileException | null) => void = () => {},
) =>
...
const make = Effect.gen(function*() {
  ...
  const buildStdioArray = (
    ...
    const maxFd = additionalFds.reduce((max, v) => Math.max(max, v.fd), 2)
  ...
  > = Effect.fnUntraced(function*(cmd) {
    switch (cmd._tag) {
      case "StandardCommand": {
        ...
        const pid = ChildProcessSpawner.ProcessId(childProcess.pid!)
        ...
        const exitCode = Effect.flatMap(Deferred.await(exitSignal), ([code, signal]) => {
          if (Predicate.isNotNull(code)) {
            return Effect.succeed(ChildProcessSpawner.ExitCode(code))
          }
...
> = Layer.effect(ChildProcessSpawner.ChildProcessSpawner, make)
