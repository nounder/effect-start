...
test.describe(Entity.EntityError, () => {
  test.it("creates a tagged error with message", () => {
    const error = new Entity.EntityError({ message: "test error" })

    test.expect(error._tag).toBe("EntityError")
...
test.describe(Entity.make, () => {
  test.it("creates entity with string body", () => {
    ...
    test.expect(entity.headers).toBeUndefined()
...
test.describe(Entity.type, () => {
  ...
  test.it("returns undefined for unknown body types", () => {
    const entity = Entity.make(null as any)

    test.expect(Entity.type(entity)).toBeUndefined()
  })
})
...
test.describe(Entity.length, () => {
  ...
  test.it("calculates JSON length for object body", () => {
    const entity = Entity.make({ a: 1 })
    const expected =
      new TextEncoder().encode(JSON.stringify({ a: 1 })).byteLength

    test.expect(Entity.length(entity)).toBe(expected)
  })
...
test.describe(Entity.text, () => {
  test.it("returns string body directly", async () => {
    const entity = Entity.make("hello world")
    const result = await Effect.runPromise(Entity.text(entity))
  ...
  test.it("fails for unsupported body types", async () => {
    ...
    const result = await Effect.runPromiseExit(Entity.text(entity))

    test.expect(result._tag).toBe("Failure")
  })
})

test.describe(Entity.json, () => {
  test.it("returns object body directly", async () => {
    const data = { key: "value", num: 42 }
    const entity = Entity.make(data)
    const result = await Effect.runPromise(Entity.json(entity))
  ...
  test.it("parses Uint8Array body as JSON", async () => {
    ...
    const result = await Effect.runPromise(
      Entity.json(entity) as Effect.Effect<unknown>,
    )
  ...
  test.it("fails for invalid JSON string", async () => {
    ...
    const result = await Effect.runPromiseExit(Entity.json(entity))
...
test.describe(Entity.bytes, () => {
  test.it("returns Uint8Array body directly", async () => {
    const bytes = new Uint8Array([1, 2, 3])
    const entity = Entity.make(bytes)
    const result = await Effect.runPromise(Entity.bytes(entity))
  ...
  test.it("fails for unsupported body types", async () => {
    const entity = Entity.make({ key: "value" })
    const result = await Effect.runPromiseExit((Entity.bytes as any)(entity))

    test.expect(result._tag).toBe("Failure")
  })
})

test.describe(Entity.stream, () => {
  test.it("returns Stream body directly", async () => {
    ...
    const result = Entity.stream(entity)

    test.expect(result).toBe(originalStream)
  })

  test.it("converts Uint8Array body to stream", async () => {
    const bytes = new Uint8Array([1, 2, 3])
    const entity = Entity.make(bytes)
    const stream = Entity.stream(entity)
...
test.describe("type inference", () => {
  ...
  test.it("Entity.text() returns string for string body", () => {
    const entity = Entity.make("hello")
    const result = Entity.text(entity)

    test.expectTypeOf<Effect.Effect.Success<typeof result>>().toEqualTypeOf<
      string
    >()
  })

  test.it("Entity.json() returns T for object body", () => {
    const entity = Entity.make({ key: "value" })
    const result = Entity.json(entity)
  ...
  test.it("Entity.bytes() returns Uint8Array for string body", () => {
    const entity = Entity.make("hello")
    const result = Entity.bytes(entity)
  ...
  test.it("Entity.stream() returns Stream for string body", () => {
    const entity = Entity.make("hello")
    const result = Entity.stream(entity)

    test.expectTypeOf<Stream.Stream.Success<typeof result>>().toEqualTypeOf<
      Uint8Array
    >()
  })
})
