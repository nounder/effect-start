...
export class StudioSupervisor extends Supervisor.AbstractSupervisor<void> {
  readonly value = Effect.void
  private readonly store: StudioStore.State

  constructor(store: StudioStore.State) {
    super()
    this.store = store
  }
    ...
    if (Exit.isFailure(exit) && !Cause.isInterruptedOnly(exit.cause)) {
      ...
      StudioStore.runWrite(
        ...
        Effect.zipRight(
          ...
          StudioStore.evict("Error", this.store.errorCapacity),
        ),
      )
      Effect.runSync(
        PubSub.publish(this.store.events, { _tag: "Error", error }),
      )
    }
  }
}
