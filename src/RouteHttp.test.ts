...
test.it(
  "returns 406 when Accept doesn't match any of multiple formats",
  async () => {
    const handler = RouteHttp.toWebHandler(
...
test.it(
  "definition order determines priority when no Accept header",
  async () => {
    const handler = RouteHttp.toWebHandler(
...
test.it(
  "falls back to html when no Accept header and no json or text",
  async () => {
    const handler = RouteHttp.toWebHandler(
...
test.describe("middleware chain", () => {
  ...
  test.it(
    "method-specific middleware enriches context for that method",
    async () => {
      const handler = RouteHttp.toWebHandler(
        Route.get(
  ...
  test.it(
    "wildcard and method-specific middlewares compose in order",
    async () => {
      const handler = RouteHttp.toWebHandler(
  ...
  test.it(
    "schema headers parsing works with HttpServerRequest service",
    async () => {
      const handler = RouteHttp.toWebHandler(
        Route.get(
...
test.describe("schema handlers", () => {
  ...
  test.it(
    "schema validation: single value with Schema.String succeeds",
    async () => {
      const handler = RouteHttp.toWebHandler(
        Route.post(
          RouteSchema.schemaBodyMultipart(
  ...
  test.it(
    "schema validation: multiple values with Schema.String fails with detailed error",
    () =>
      Effect.gen(function* () {
        const runtime = yield* Effect.runtime<TestLogger.TestLogger>()
        const handler = RouteHttp.toWebHandlerRuntime(runtime)(
  ...
  test.it(
    "composes shared middleware with method-specific schema",
    async () => {
      const handler = RouteHttp.toWebHandler(
        Route.use(
          RouteSchema.schemaHeaders(
...
test.describe("request abort handling", () => {
  test.it(
    "returns 499 and runs finalizers when request is aborted",
    async () => {
  ...
  test.it(
    "uses clientAbortFiberId to identify client disconnects",
    async () => {
...
test.describe("Route.render (format=*)", () => {
  ...
  test.it(
    "render middleware always runs even when specific format is selected",
    async () => {
      const calls: string[] = []
  ...
  test.it(
    "next() from render matches both render and selected format routes",
    async () => {
      const calls: string[] = []
  ...
  test.it(
    "render used as fallback when Accept doesn't match other formats",
    async () => {
      const handler = RouteHttp.toWebHandler(
        Route.get(
