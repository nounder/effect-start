...
export const boolean: Primitive<boolean> = make("Boolean", (value) => {
  ...
  return Effect.fail(
    `Expected boolean (true/false/yes/no/on/off/1/0), got "${value}"`,
  )
})
...
export const integer: Primitive<number> = make("Integer", (value) => {
  ...
  if (!Number.isInteger(n)) {
    return Effect.fail(`Expected integer, got "${value}"`)
  }
...
export const date: Primitive<Date> = make("Date", (value) => {
  ...
  if (Number.isNaN(d.getTime())) {
    return Effect.fail(`Expected valid date, got "${value}"`)
  }
...
export const none: Primitive<never> = make(
  "None",
