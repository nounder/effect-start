...
const toPgUnsafeValues = (
  ...
  values === undefined
    ? undefined
    : (values as unknown as Array<Postgres.ParameterOrJSON<never>>)
        ...
        try: () => {
          ...
          if (Option.isSome(txOpt)) {
            return Promise.resolve(
              txOpt.value.conn<ReadonlyArray<T>>(
                strings,
                ...args,
          ...
          return Promise.resolve(
            pg<ReadonlyArray<T>>(strings, ...args) as unknown as ReadonlyArray<
              T
            >,
          )
        },
    ...
    (txOpt) =>
      Effect.tryPromise({
        try: () => {
          ...
          if (Option.isSome(txOpt)) {
            return Promise.resolve(
              txOpt.value.conn.unsafe<Array<T>>(
                query,
                args,
              ) as unknown as ReadonlyArray<T>,
            )
          ...
          return Promise.resolve(
            pg.unsafe<Array<T>>(query, args) as unknown as ReadonlyArray<T>,
          )
        },
        catch: wrapError,
      }),
  ...
  Layer.scoped(
    ...
    Effect.map(
      Effect.acquireRelease(
        Effect.try({
          try: () => {
            const pg = typeof config === "string"
              ? postgres(config)
              : postgres(
                config as Postgres.Options<
