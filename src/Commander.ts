...
const optionMethod = function <S, Opt extends OptionBuilder<any, any>>(
  ...
  const base = (
    this && typeof this === "object" ? this : {}
  ) as Partial<CommanderSet.Instance>
...
>(config: {
  ...
  return Object.assign(Object.create(CommanderProto), config) as CommanderSet<
    Opts,
    Subs,
    Handled
  >
    ...
    for (const optBuilder of Object.values(cmd.options)) {
      ...
      if (rawValue !== undefined) {
        if (typeof rawValue === "boolean") {
          ...
          const decoded = yield* Schema.decode(optBuilder.schema)(
            rawValue,
          ).pipe(
            Effect.mapError(
              (error) =>
