...
test.describe(`${EncryptedCookies.encrypt.name}`, () => {
  ...
  test.test(
    "produce different results for same input due to random IV",
    async () => {
...
test.describe("service", () => {
  ...
  test.test(
    "service cookie functions work with pre-calculated key",
    async () => {
      const testSecret = "test-secret-key"
      const originalCookie = Cookies.unsafeMakeCookie("test", "hello world")
  ...
  test.test(
    "functions work with pre-derived keys passed as options",
    async () => {
      const testSecret = "test-secret-key"
      const testValue = "hello world"
