...
const getStatus = (command: string) =>
  Effect.gen(function*() {
    ...
    const json = yield* Schema
      .decodeUnknown(Schema.parseJson(TailscaleStatus))(stdout)
      .pipe(
        Effect.mapError(
          (cause) =>
            new TailscaleError({
    ...
    if (json.BackendState !== "Running") {
      return yield* new System.SystemError({
        ...
        method: "getStatus",
        description:
          `tailscale is in state "${json.BackendState}", expected "Running"`,
      })
    }
        ...
        Effect
          .gen(function*() {
            ...
            yield* Effect.logInfo(
              `Tailscale ${opts?.public ? "funnel" : "serve"}${
                serveUrl ? ` url=${serveUrl}` : ""
              }`,
            )
          ...
          .pipe(
            Effect.tapError((e) => Effect.logError(`Tailscale: ${e.message}`)),
          ),
      )
    }),
  )
